...
def build(preview=False):
 ...
 for ident,p in papers.items():
  path='papers/'+slug(ident)+'/';ps=paragraphs(p['note']);summary=ps[0] if ps else plain(p['note'])[:250]
  e={'id':ident,'type':'paper','title':p['title'],'date':p['date'],'dates':p['dates'],'url':p['url'],'category':'论文 / '+next((t for t in p.get('tags',[]) if t not in ['待阅读','补看','Juju Radar']),'研究'),'topics':topics(' '.join(p.get('tags',[]))+' '+p['title']),'summary':summary,'note':p['note'],'path':PREFIX+path,'authors':p.get('authors',[]),'backfill':'补看' in p.get('tags',[])}
  ...
  body=f'<main class="reading"><a class="back" href="{PREFIX}?view=library&type=paper">论文库</a><h1>{E(p["title"])}</h1><p class="meta">首次公开：{E(p.get("published_date","日期待补"))} · 收录：{E("、".join(p["dates"]))}</p>'+(f'<details><summary>作者</summary><p class="meta">{E(author)}</p></details>' if author else '')+f'<div class="actions"><a href="{E(p["url"])}" target="_blank" rel="noopener noreferrer">论文原文 {ARROW}</a>'+(f'<a href="{E(p["pdf_url"])}" target="_blank" rel="noopener noreferrer">PDF {ARROW}</a>' if p.get('pdf_url') else '')+f'<a href="{PREFIX+path}citation.ris" download>导入 Zotero（RIS）</a></div><h2>阅读笔记</h2>{md(note)}<p class="notice">笔记记录收录当期的判断。阅读一手材料不等于独立复现；后续进展请以原始来源为准。</p></main>'
  save(path+'index.html',shell(p['title'],body,path))
 for e in projects.values():
  entries.append(e);path=e['path'].removeprefix(PREFIX)
  save(path+'index.html',shell(e['title'],f'<main class="reading"><a class="back" href="{PREFIX}?view=library&type=project">项目库</a><h1>{E(e["title"])}</h1><p class="meta">收录：{E("、".join(e["dates"]))}</p><p class="lead">{E(e["summary"])}</p><div class="actions"><a href="{E(e["url"])}" target="_blank" rel="noopener noreferrer">GitHub 仓库 {ARROW}</a><a href="{PREFIX}read/{e["date"]}/">查看本期文章</a></div><p class="notice">仓库状态可能随时间变化，开放范围与使用条件请查看项目自身的许可证和说明。</p></main>',path))
 ...
 body=f'<main><h1>AI 精选</h1><p class="intro">每日 AI 动态、论文和开源项目，附中文解读与原始链接。</p><div class="meta">{latest.replace("-",".")} 更新 · {len(issues)} 期归档 · {len(papers)} 篇论文</div><div class="filters" role="search"><div class="search"><label class="sr-only" for="query">搜索资料</label><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><circle cx="10" cy="10" r="7"/><path d="m15 15 6 6"/></svg><input id="query" type="search" placeholder="搜索论文、项目、关键词…" autocomplete="off"></div><label class="sr-only" for="type">类型</label><select id="type"><option value="">全部类型</option><option value="news">精选</option><option value="paper">论文</option><option value="project">项目</option></select><label class="sr-only" for="topic">主题</label><select id="topic"><option value="">全部主题</option>{topicopts}</select></div><div class="layout"><section aria-label="精选资料"><div class="section-head"><h2 class="section-title" id="results-title">最新精选</h2><p class="meta" id="result-count" role="status" aria-live="polite">{latest}</p></div><div id="results">{cards}</div></section>{sidebar}</div></main>'
 ...
 urls=['', 'archive/','subscribe/']+['read/'+i['date']+'/' for i in issues]+[e['path'].removeprefix(PREFIX) for e in entries if e['type']!='news']
