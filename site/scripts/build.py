...
def build(preview=False):
 ...
 def shell(title,body,canonical='',script='',description=''):
  description=description or '每日 AI 动态、论文和开源项目，附中文解读与原始链接。'
 ...
 for folder in folders:
  ...
  text=re.sub(r'^---\n.*?\n---\n','',text,flags=re.S)
  save('read/'+date+'/index.html',shell(title,f'<main class="reading"><a class="back" href="{PREFIX}archive/">全部往期</a>{md(text)}<div class="actions"><a href="{BASE+date}/">本期全部原始资料 {ARROW}</a><a href="{PREFIX}?date={date}">本期精选列表</a></div></main>','read/'+date+'/',description=desc))
 ...
 paper_cards=''.join(f'<article class="entry"><div class="meta">{E(e["category"])} · {e["date"]} 收录</div><h2><a href="{e["path"]}">{E(e["title_zh"])}</a></h2><p class="paper-original">{E(e["title"])}</p><p>{E(e["summary"])}</p><div class="links">{share_button(e.get("title_zh",e["title"]),e["path"])}<a href="{e["path"]}">阅读笔记 {ARROW}</a><a href="{E(e["url"])}" target="_blank" rel="noopener noreferrer">论文原文 {ARROW}</a></div><div class="paper-issues meta">相关期次：'+ ' · '.join(f'<a href="{PREFIX}read/{d}/">{d}</a>' for d in e['dates'])+'</div></article>' for e in entries if e['type']=='paper')
 ...
 for issue in issues:
  ...
  archive_groups.append(f'<section class="archive-group" id="date-{issue["date"]}" data-date="{issue["date"]}"><div class="archive-heading"><h2>{issue["date"]}</h2><span class="meta">{len(daily)} 条精选</span><a href="{issue["path"]}">阅读整期 {ARROW}</a>{share_button(issue["title"],issue["path"],"分享整期")}</div><p class="archive-issue-title">{E(issue["title"])}</p><ol class="archive-items">{rows}</ol></section>')
